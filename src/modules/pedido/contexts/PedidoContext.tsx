...
export const PedidoProvider: React.FC<PedidoProviderProps> = ({ children }) => {
    ...
    function deletarItem(produtoId: number){
        if (carrinho){
            let auxCarrinho = carrinho.itens?.filter((itemCarrinho: ItemCarrinhoInterface) => itemCarrinho.produto.id === produtoId )
    ...
    async function adicionarCupom(codigo: string){
        const cuponsDisponiveis = cupons?.filter((cupom: CupomInterface) => cupom.status && cupom.status === StatusCupom.DISPONIVEL)
    ...
    function alterarQuantidade(produtoId: number, quantidade: number) {
      ...
      async function salvarPedido(){
            ...
            console.log('nvo pedido', novoPedido)
